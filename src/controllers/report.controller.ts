...
export class ReportController {
  ...
  @Get('/joiners/:joinerId/tasks')
  async taskCompletedAndUncompletedFilterByJoiner(@Res() res: Response, @Param('joinerId') joinerId): Promise<void> {
    const fileName = await this.reportService.taskCompletedAndUncompletedFilterByJoiner(joinerId);
    res.download(`./files/${fileName}.csv`);
  }
